        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equalsIgnoreCase("") && !sender.hasPermission(command.permission())) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', command.noPermission()));

                    if (command.inGameOnly()) {
                        Player player = (Player) sender;
                        player.playSound(player.getLocation(), Sound.ENTITY_WOLF_AMBIENT, 20f, 20f);
                    }
